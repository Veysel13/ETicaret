...
let TYPE_TEXT = "TEXT",
    ...
    Logger = {
        logScanStart: function (e) {
            //gtag("event", "ScanStart", { event_category: "Start", event_label: `embed=${e}` });
        },
        logScanRestart: function () {
            //gtag("event", "ScanStart", { event_category: "Restart", event_label: "NA" });
        },
        logScanSuccess: function (e, t) {
            //gtag("event", "ScanSuccess", { event_category: e, event_label: t }), gtag("event", `ScanSuccess_${e}`, { event_category: "codeType", event_label: "NA" });
        },
...
function showBanner(e, t) {
    ...
    !1 === t && ((selector = ".banner.error"), (textId = "banner-error-message")),
...
let QrResult = function (e) {
    let t = document.getElementById("new-scanned-result"),
        ...
        p = document.getElementById("scan-result-close-window"),
    ...
    u.addEventListener("click", function () {
        ...
        var saleOrderBarcode=document.getElementById("saleOrderBarcode").value
        ...
        if (saleOrderBarcode){
            ...
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    var response=JSON.parse(this.response);
                    showBanner(response.message??'Unexpected Error',!1)
                    setTimeout(function (){
                        hideBanners()
                    },2000)
                }
            ...
            xhttp.open("POST","/backend/code/scann", true);
            xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhttp.send("text="+g.text+"&type="+g.type+"&saleOrderBarcode="+saleOrderBarcode);
        }else{
            showBanner('Product Not Imported.Enter the order barcode number',!1)
            setTimeout(function (){
                hideBanners()
            },3000)
        }
                    ...
                    (function (e, t) {
                        ...
                        return (
                            (s.style.display = t == TYPE_UPI ? "inline-block" : "none"),
                                t == TYPE_URL || t == TYPE_PHONE ? (createLinkTyeUi(n, e, t), n) : t == TYPE_WIFI ? (createWifiTyeUi(n, e), n) : t == TYPE_UPI ? (createUpiTypeUi(n, e), n) : ((n.innerText = e), n)
                        );
                    })(e, d)
                ),
...
function productInfo(text){
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response.status){
                ...
                const productImg=response.result.image;
                let productIndoTd = document.getElementById("product-info");

                let productInfoText='<hr><div style="text-align: center;">Name: '+product.name+'</div><div style="text-align: center;">Sku: '+product.sku+'</div><div><center><img style="width: 90px;height: 90px;text-align: center" src="'+productImg+'" alt=""></center></div>'

                productIndoTd.innerHTML=productInfoText

            }
        }else{
            let productIndoTd = document.getElementById("product-info");

            let productInfoText='<hr><div style="text-align: center;color: red">Product Not Found</div>'
    ...
    xhttp.open("GET","/backend/code/product?text="+text, true);
    xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhttp.send();
}
    ...
    docReady(function () {
        ...
        let t = new Html5QrcodeScanner("reader", {
                ...
                qrbox: function (e, t) {
                    var n = e > t ? t : e,
                        o = Math.floor(0.8 * n);
                    return o < 250 ? (n < 250 ? { width: n, height: n } : { width: 250, height: 250 }) : { width: o, height: o };
                },
