...
//        $couponRedis->removeCouponGroup($couponGroupId);
    ...
    public function storeCouponRedis(Request $request, $couponGroupId)
    {
        set_time_limit(0);
        $request->validate([
            'coupon_count' => 'required|numeric|min:1',
        ...
        if ($coupon_type === 'txtFile') {
            ...
            if (count($expContents) > 10000) {

                $c = 0;
                $step = 5000;
                for ($i = 0; $i < 5000; $i++) {
                    $list = array_slice($expContents, $c, $step);
                    $c = $c + $step;
                    ...
                    $store = [
                        ...
                        'gsm' => null,
                        'status' => 0,
                        'user_list' => json_encode($list)
                    ];
                    if (sizeof($list)) {
                        CouponJobs::insert($store);
                    }
                }
                dispatch(new CouponCodeAdd());
                return redirect()
                    ->route('backend.couponManagement.showGroup', $couponGroupId)
                    ->with('success', 'Kuponlar yüklenmek üzere kuyruğa alındı.');
            }
        ...
        if ($coupon_count > 10000) {
            ...
            dispatch(new AddCouponCode($coupon_count, 1, $couponCodeData));

            session()->flash("success", "Kupon kodları ekleniyor.İşlem tamamlandıkdan sonra kupon detayında inceleyebilirsiniz.");

            return redirect()->route('backend.couponManagement.showGroup', $couponGroupId);
        }
